...
function csvEscape(v) {
  const s = String(v ?? "");
  // ถ้ามี , " \n \r ต้องครอบด้วย " และ escape " เป็น ""
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ---- Return URL (choose one) ----
    // A) signed url (ใช้ได้แม้ bucket private)
    ...
    return res.json({
      ok: true,
      saved_to: filePath,
      signed_url: data?.signedUrl, // เปิดดู/ดาวน์โหลดเองถ้าต้องการ
    });

    // B) ถ้า bucket เป็น public อยู่แล้ว ใช้ public url แทนได้ (ไม่หมดอายุ)
    // const { data: pub } = supabase.storage.from("csv").getPublicUrl(filePath);
    // return res.json({ ok: true, saved_to: filePath, public_url: pub?.publicUrl });
